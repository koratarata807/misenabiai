...
import { buildInsightPrompt, fallbackGeneratedInsight } from "@/lib/ai-insights/prompts";
import { generateInsightWithLLM } from "@/lib/ai-insights/llm";
...
export async function GET(req: NextRequest) {
  ...
  // v1.2：前週比較はまだ無し（後で7d/prev7d集計で実装）
  ...
  let generated = fallbackGeneratedInsight(decision);
  ...
  try {
    ...
    generated = await generateInsightWithLLM({ prompt, shopId });
  } catch (e) {
    console.error("[ai-insights] LLM generation failed:", e);
    // generated はフォールバックのまま
  }
